...
from weather_context import weather_risk
...
CROP_BN={
    "Tomato":"টমেটো","Potato":"আলু","Pepper  bell":"ক্যাপসিকাম","Pepper bell":"ক্যাপসিকাম",
...
def display_crop_disease(crop,disease,lang):
    ...
    return CROP_BN.get(crop,crop),DISEASE_BN.get(disease,disease)
...
with st.sidebar:
    ...
    st.divider(); threshold=st.slider(t["sharp"],20,300,100); st.caption("AgroCare • Final Integrated")
...
if page==t["home"]:
        ...
        if img is None: st.error(t["badimg"])
            ...
            with left:
                ...
                if ok:
                    ...
                    st.markdown(f'<div class="scan-frame"><img src="data:image/jpeg;base64,{b64}"><div class="laser"></div><div class="scan-label">◉ AI VISION SCAN • {t["preview"]}</div></div>', unsafe_allow_html=True); q=check_image_quality(img,threshold); st.markdown(f'<div class="card"><b>{t["quality"]}</b><div class="metric">{q["score"]:.1f}</div><span class="muted">{t["score"]}</span></div>',unsafe_allow_html=True)
            with right:
                model_path=Path("models/best_model.joblib")
                if not q["is_sharp"]: st.warning(t["blurry"])
                elif not model_path.exists(): st.warning(t["nomodel"]); st.write(t["copy"])
                else:
                    saved=joblib.load(model_path); model=saved["model"] if isinstance(saved,dict) and "model" in saved else saved; feat=np.array(extract_features(img),dtype=float).reshape(1,-1); pred=model.predict(feat)[0]; conf=None; probs=None
                    if hasattr(model,"predict_proba"): probs=model.predict_proba(feat)[0]; conf=float(np.max(probs))
                    cutoff=st.slider(t["reliable"],0.30,0.95,0.55,0.05)
                    if conf is not None and conf<cutoff:
                        st.markdown(f'<div class="warn"><b>{t["unknown"]}</b><br>{t["unknown_d"]}</div>',unsafe_allow_html=True); st.metric(t["best"],f"{conf*100:.1f}%"); upload_key=f"{up.name}:{up.size}:{conf:.6f}:unknown"
                    ...
                    if probs is not None:
                        classes=model.classes_; top=np.argsort(probs)[::-1][:5]; st.markdown("#### "+t["matches"]); st.dataframe(pd.DataFrame({t["class"]:[display_class_label(classes[i],lang) for i in top],t["prob"]:[f"{probs[i]*100:.2f}%" for i in top]}),use_container_width=True,hide_index=True)
elif page==t["weather"]:
    st.subheader(t["weather_title"]); c1,c2,c3=st.columns(3); temp=c1.number_input(t["temp"],value=28.0); hum=c2.number_input(t["hum"],0,100,75); rain=c3.number_input(t["rain"],0.0,500.0,0.0); level,reasons=weather_risk(temp,hum,rain)
    ...
    st.subheader(t["history_title"]); summary=get_summary(); a,b,c,d=st.columns(4); a.metric(t["total"],summary["total"]); b.metric(t["detected"],summary["detected"]); c.metric(t["unknown_m"],summary["unknown"]); d.metric(t["avg"],f'{summary["avg_confidence"]:.1f}%'); rows=get_history(200)
    if rows:
        df=pd.DataFrame(rows);
        if lang=="বাংলা":
            df["crop"]=df["crop"].map(lambda x: CROP_BN.get(str(x), "অজানা" if str(x)=="Unknown" else x))
            df["disease"]=df["disease"].map(lambda x: DISEASE_BN.get(str(x), "সমর্থিত নয়" if str(x)=="Unsupported" else x))
